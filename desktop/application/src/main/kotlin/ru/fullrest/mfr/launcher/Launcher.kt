package ru.fullrest.mfr.launcher
...
import ru.fullrest.mfr.launcher.config.FxConfiguration
import ru.fullrest.mfr.launcher.exception.OnlineModException
import ru.fullrest.mfr.launcher.exception.StartApplicationException
import java.io.File
import java.util.*
...
@EntityScan(basePackages = ["ru.fullrest.mfr.launcher.model.entity"])
@EnableJpaRepositories(basePackages = ["ru.fullrest.mfr.launcher.model.repository"])
@ComponentScan
...
class Launcher : Application() {
    ...
    private fun initSpringBoot() = CoroutineScope(Dispatchers.Default).launch {
        try {
            ...
            runApplication<Launcher>(*parameters.raw.toTypedArray() + initExternalConfiguration())
                .registerShutdownHook()
            log.info("Spring Boot initialization completed")
        } catch (e: OnlineModException) {
            throw e
        } catch (e: Exception) {
